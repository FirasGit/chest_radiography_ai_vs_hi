...
def get_best_checkpoint(checkpoint_dir, metric='Val_mean_auc', mode='max'):
    ...
    if os.path.exists(checkpoint_dir):
        ...
        if checkpoint_files:
            resume_from_checkpoint = _get_filename_with_best_score(checkpoint_files, metric, mode)
...
def _get_filename_with_best_score(checkpoint_files, metric, mode):
    ...
    try:
        if mode == 'max':
            best_file_idx = np.nanargmax(scores)    # ignores nan when looking for argmax
        elif mode == 'min':
